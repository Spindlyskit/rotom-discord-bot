...
module.exports = class DriverpromoteCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'driver',
            group: 'mod',
            memberName: 'driver',
            description: 'Promote a user to driver.\nRequires @, &, ~',
            examples: ['driver Spindlyskit'],
            aliases: ['globaldriver'],
            args: [
                {
                    key: 'member',
    ...
    run(msg, { member }) {
        ...
        for (let i=0; i<hierarchy.length; i++) {
            ...
            if (member.roles.has(tr) && tr != role) {
                member.removeRole(tr, `${msg.author.tag} used driver command in ${msg.channel.name}`)
                .catch(console.error);
            }
        }
        member.addRole(role, `${msg.author.tag} used driver command in ${msg.channel.name}`)
