...
module.exports = class ModpromoteCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'mod',
            group: 'mod',
            ...
            examples: ['mod Spindlyskit'],
            aliases: ['globalmod', 'moderator', 'globalmoderator'],
            args: [
                {
                    key: 'member',
    ...
    run(msg, { member }) {
        ...
        for (let i=0; i<hierarchy.length; i++) {
            ...
            if (member.roles.has(tr) && tr != role) {
                member.removeRole(tr, `${msg.author.tag} used mod command in ${msg.channel.name}`)
                .catch(console.error);
            }
        }
        member.addRole(role, `${msg.author.tag} used mod command in ${msg.channel.name}`)
